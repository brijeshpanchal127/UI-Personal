...
import {
  selectDisplayStoreSelection,
  selectDisplayStoreFunctionalities,
} from "../../../reducers/actions/display.action";

const Landing = () => {
  const dispatch = useDispatch();
  const auth = useSelector((state) => state.login.auth);
  const selectedStore = useSelector((state) => state.landing.selectedStore);
  const currentStoreFunction = useSelector(
    (state) => state.landing.currentStoreFunction
  );
